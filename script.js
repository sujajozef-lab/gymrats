/* ═══════════════ AUTH ═══════════════ */
function authHash(s){return btoa(encodeURIComponent(s));}
function authGetUsers(){try{return JSON.parse(localStorage.getItem('gp5_users')||'[]');}catch(e){return[];}}
function authSaveUsers(u){localStorage.setItem('gp5_users',JSON.stringify(u));}
function authCurrentUser(){return localStorage.getItem('gp5_currentUser');}

function authCheck(){
  var overlay=document.getElementById('auth-overlay');
  if(!overlay) return;
  overlay.style.display=authCurrentUser()?'none':'flex';
}
...
function authLogin(){
  ...
  var user=users.find(function(u){return u.email===email&&u.hash===authHash(pass);});
...
function authRegister(){
  ...
  var users=authGetUsers();
  if(users.find(function(u){return u.email===email;})){err.textContent='Email already registered.';return;}
  users.push({email:email,hash:authHash(pass),name:name});
  authSaveUsers(users);
  localStorage.setItem('gp5_currentUser',email);
  document.getElementById('auth-overlay').style.display='none';
}
